...
export default class Todolist extends React.PureComponent<TodoListProps,TodoListState> {
	...
	componentDidMount () {
		this.store.addTodo('find a job')
		// this.store.addTodo('go to Peru')
	}
	render () {
		...
		return <section className="todoapp">
					...
					<footer className="footer">
						{remainingCount > 0 && <span className="todo-count"><strong>{remainingCount}</strong> item{remainingCount > 1 && 's'} left
						</span>}
						<ul className="filters">
							<li>
								<a href="#/all" 
								className={ cx({ selected: filter === 'all' }) } 
								onClick={this.setFilter('all')}>All</a>
							</li>
							<li>
								<a href="#/active" 
